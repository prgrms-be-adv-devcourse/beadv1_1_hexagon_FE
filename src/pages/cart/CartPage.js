...
const CartPage = () => {
  ...
  if (loading)
    return (
      <div className="p-8 text-center text-gray-600">장바구니 로딩 중...</div>
    );
      ...
      <div className="space-y-4">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div
              key={item.itemCode}
              className="flex justify-between items-center p-4 bg-gray-50 border rounded-lg hover:shadow-md transition duration-200"
            >
              <span className="text-lg font-semibold text-gray-700">
                {item.title} -{" "}
                <span className="text-blue-600 font-bold">{item.amount}원</span>
              </span>
              <button
