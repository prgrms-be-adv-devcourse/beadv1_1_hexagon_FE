...
const MemberRatingPage = () => {
    ...
    const [rating, setRating] = useState({ satisfiedCount: 0, unsatisfiedCount: 0 });
    ...
    if (loading) return <div className="p-8 text-center text-gray-600">회원 평가 로딩 중...</div>;

    const totalCount = rating.satisfiedCount + rating.unsatisfiedCount;
    const satisfiedRatio = totalCount > 0 ? (rating.satisfiedCount / totalCount) * 100 : 0;

    return (
        <div className="max-w-lg mx-auto p-8 bg-white shadow-2xl rounded-xl text-center">
            <h2 className="text-3xl font-bold mb-8 text-gray-800">⭐ 회원 평가: {memberCode}</h2>
            
            <div className="rating-summary mb-8 p-6 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-lg font-semibold text-gray-700">총 평가: <span className="text-blue-600 font-extrabold">{totalCount} 건</span></p>
                <div className="flex justify-around mt-4">
                    <p className="text-2xl font-bold text-green-600">
                        만족: <span className="font-extrabold">{rating.satisfiedCount}</span>
                    </p>
                    <p className="text-2xl font-bold text-red-600">
                        불만족: <span className="font-extrabold">{rating.unsatisfiedCount}</span>
                    </p>
                </div>
                ...
                <p className="text-sm text-gray-500 mt-2">만족도: {satisfiedRatio.toFixed(1)}%</p>
            </div>
            
            <div className="rating-actions">
                <h3 className="text-xl font-semibold mb-4 text-gray-700">이 회원에 대해 평가하기:</h3>
